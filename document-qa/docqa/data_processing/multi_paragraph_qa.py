...
class WeightedMultiParagraphQuestion(MultiParagraphQuestion):
    """ Question associated with multiple paragraph w/pre-computed answer spans and a sample weight"""

    __slots__ = ["question_id", "question", "weight", "end", "answer_text", "paragraphs"]

    def __init__(self, question_id: str, question: List[str], answer_text: List[str],
            paragraphs: List[ParagraphWithAnswers], weight: float):
        super().__init__(question_id, question, answer_text, paragraphs)
        self.weight = weight
...
class StratifyParagraphsDataset(Dataset):
    ...
    def get_epoch(self):
        return self.build_epoch(self.questions)

    def build_epoch(self, questions):
        out = []
...
class WeightedStratifyParagraphsDataset(StratifyParagraphsDataset):
    """
    Samples paragraph for each question to build question-paragraph pairs, but
    stratify the sampling across epochs so paragraphs are seen at about the same rate
    Also supports weighted samples by dropping samples proportional to their weight
    """
    ...
    def __init__(self,
                 ...
                 batcher: ListBatcher):
        super().__init__(questions, true_len, oversample_first_answer, batcher)

    def get_epoch(self):
        questions = drop_weighted_questions(self.questions)
        return self.build_epoch(questions)
...
class WeightedStratifiedParagraphSetDataset(StratifiedParagraphSetDataset):
    """
    Sample multiple paragraphs each epoch and include them in the same batch,
    but stratify the sampling across epochs. Drop samples according to their
    weight
    """

    def __init__(self,
                 questions: List[WeightedMultiParagraphQuestion],
                 true_len: int,
                 batch_size: int,
                 force_answer: bool,
                 oversample_first_answer: List[int],
                 merge: bool):
        super().__init__(questions, true_len, batch_size, force_answer, oversample_first_answer, merge)


    def get_epoch(self):
        questions = drop_weighted_questions(self.questions)
        return self._build_expanded_batches(questions)

def drop_weighted_questions(questions: List[WeightedMultiParagraphQuestion]):
    """
    Takes a list of weighted questions, drops some of them proportional to their weight
    """
    return [question for question in questions if question.weight > np.random.random()]
...
class WeightedStratifyParagraphsBuilder(StratifyParagraphsBuilder):
    def __init__(self, batcher: ListBatcher,  oversample_answers: Union[int, List[int]],
                 only_answers: bool=False):
        self.batcher = batcher
        self.oversample_answers = oversample_answers
    ...
    def build_dataset(self, data, evidence) -> Dataset:
        ...
        if isinstance(self.oversample_answers, int):
            ...
            ov = self.oversample_answers
        return WeightedStratifyParagraphsDataset(data, l, ov, self.batcher)
...
class WeightedStratifyParagraphSetsBuilder(DatasetBuilder):
    def __init__(self, batch_size: int, merge: bool, force_answer: bool,
                 oversample_first_answer: Union[int, List[int]]):
        self.batch_size = batch_size
        self.merge = merge
    ...
    def build_dataset(self, data: Union[FilteredData, List], corpus) -> Dataset:
        ...
        if isinstance(self.oversample_first_answer, int):
            ...
            ov = self.oversample_first_answer
        return WeightedStratifiedParagraphSetDataset(data, l, self.batch_size, self.force_answer,
                                             ov, self.merge)
