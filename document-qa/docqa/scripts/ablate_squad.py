...
from docqa.data_processing.multi_paragraph_qa import (
    StratifyParagraphSetsBuilder,
    StratifyParagraphsBuilder,
    WeightedStratifyParagraphSetsBuilder,
    WeightedStratifyParagraphsBuilder,
...
from docqa.squad.squad_document_qa import SquadTfIdfRanker, SquadDefault, SquadWeighted
...
def main():
    ...
    parser.add_argument("--weighted-questions", action='store_true', help="Read a weighted training dataset")
    ...
    if mode == "paragraph":
        ...
        if args.no_tfidf:
            ...
            if args.weighted_questions:
                raise NotImplementedError("Weighted questions not supported for tf-idf mode yet")
        ...
        if mode == "confidence" or mode == "sigmoid":
            ...
            if args.weighted_questions:
                dataset_builder = WeightedStratifyParagraphsBuilder(train_batching, 1)
            else:
            ...
            n_epochs = 26
            if args.weighted_questions:
                dataset_builder = WeightedStratifyParagraphSetsBuilder(25, args.mode == "merge", True, 1)
            else:
                dataset_builder = StratifyParagraphSetsBuilder(25, args.mode == "merge", True, 1)
